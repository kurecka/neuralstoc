...
class vLDSEnv(gym.Env):
    ...
    def reset(self, rng):
        ...
        i = jax.random.randint(rng, (), 0, len(self.init_spaces))
        obs = init_samples[i]
...
class vHumanoidBalance2(gym.Env):
    name = "vhuman2"

    def __init__(self):
        self.episode_length = 200
        self.plot_dims = (0, 2)
        self.plot_dim_map = {1: 0, 3: 2}
    ...
    def step(self, state, action, rng):
        ...
        next_step = jnp.minimum(next_step, self.episode_length)
...
class vLDSS(gym.Env):
    def __init__(self, num_dims=2):
        ...
        self.episode_length = 200
        self.plot_dims = (0, num_dims // 2)
        self.plot_dim_map = {i: 0 for i in range(1, num_dims // 2)} | {
            i: num_dims // 2 for i in range(num_dims // 2 + 1, num_dims)
        }
        ...
        self.noise = np.array([0.001] * (num_dims // 2) + [0] * (num_dims // 2))
        self.unsafe_spaces = [
            spaces.Box(
                low=self.observation_space.low,
    ...
    def step(self, state, action, rng):
        ...
        next_step = jnp.minimum(next_step, self.episode_length)
        next_packed = jnp.array(
            [next_step] + [next_state[i] for i in range(self.num_dims)]
        )
        return next_packed, next_state, reward, done
...
class vTripleIntegrator(gym.Env):

    def __init__(self):
        self.name = f"vtri"
        self.episode_length = 200
        self.plot_dims = (0, 2)
        self.plot_dim_map = {1: 0}
    ...
    def step(self, state, action, rng):
        ...
        next_step = jnp.minimum(next_step, self.episode_length)
        next_packed = jnp.array(
            [next_step] + [next_state[0], next_state[1], next_state[2]]
        )
        return next_packed, next_state, reward, done
...
class v2CollisionAvoidanceEnv(gym.Env):
    name = "v2cavoid"
...
class v2InvertedPendulum(gym.Env):
    def __init__(self):
        self.name = "v2pend"
        self.episode_length = 200
        self.plot_dims = (0, 1)
...
class v2LDSS(gym.Env):
    ...
    @property
    def lipschitz_constant(self):
        A = np.max(np.sum(np.array([[1, 1, 0.045, 0.045, 0.45, 0.45]] * (self.num_dims // 2) +
                                   [[0, 0, 0.9, 0.9, 0.5, 0.5]] * (self.num_dims // 2)), axis=0))
        return A

    @property
    def lipschitz_constant_linf(self):
        A = np.max(np.sum(np.array([[1, 1, 0.045, 0.045, 0.45, 0.45]] * (self.num_dims // 2) +
                                   [[0, 0, 0.9, 0.9, 0.5, 0.5]] * (self.num_dims // 2)), axis=1))
        return A
