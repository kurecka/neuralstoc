...
from brax.training.gradients import loss_and_pgrad
...
def gradient_update_fn_adamw(loss_fn: Callable[..., float],
  ...
  def f(params, *args, optimizer_state):
    value, grads = loss_and_pgrad_fn(*args)
    params_update, optimizer_state = optimizer.update(grads, optimizer_state, params=params)
...
def train(
  ...
  sac_network = network_factory(
      ...
      hidden_layer_sizes=p_hidden,
      preprocess_observations_fn=normalize_fn)
  make_policy = sac_networks.make_inference_fn(sac_network)

  alpha_optimizer = optax.adam(learning_rate=3e-4)

  policy_optimizer = optax.adamw(learning_rate=learning_rate, weight_decay=weight_decay)
  ...
  actor_update = gradient_update_fn_adamw(  # pytype: disable=wrong-arg-types  # jax-ndarray
        actor_loss, policy_optimizer, pmap_axis_name=_PMAP_AXIS_NAME)
